...
public class ListGymsQueryHandler : IRequestHandler<ListGymsQuery, ErrorOr<List<Gym>>>
{
    ...
    {
        if (!await _subscriptionsRepository.SubscriptionExistsAsync(query.SubscriptionId))
        {
            return Error.NotFound(description: "Subscription not found");
        }

        return await _gymsRepository.ListGymsBySubscriptionIdAsync(query.SubscriptionId);
    }
}